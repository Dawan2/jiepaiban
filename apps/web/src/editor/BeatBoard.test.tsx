...
describe('画面宫格：格数由板位锁定（AC-6.3）', () => {
  it.each<[BeatIndex, number]>([
    ...
    expect(FRAME_COUNT_BY_BEAT_INDEX[index]).toBe(expected);
  ...
  it('五板格数合计 14', async () => {
    ...
    expect(total).toBe(TOTAL_FRAME_COUNT);
  });
...
describe('信息条：情绪可改、时间位派生、参数位不入文本', () => {
  ...
  it('时间位是 canon 规格，只读呈现（METH-003 §1）', async () => {
    await selectBeat(1);
    const infobar = () => within(screen.getByRole('region', { name: '节拍信息条' }));
    // 时间位 0-8 / 8-25 / 25-45 / 45-70 / 70-88 由 BEAT_DEFS 锁定，不由时长推算。
    expect(infobar().getByText('时间位').textContent).toContain('锁定');
    expect(infobar().getByText('0–8s')).toBeInTheDocument();

    await selectBeat(2);
    expect(infobar().getByText('8–25s')).toBeInTheDocument();
  });

  it('时长可改，既进 Prompt 文本也进参数位（METH-003 §2 / PRD 5.3.2）', async () => {
    await selectBeat(1);
...
describe('组间衔接：模板齐备，且不参与 AI 生成（AC-6.4）', () => {
  it('B1–B4 提供 6 个衔接模板，并明示不参与 AI 生成', async () => {
    ...
    TRANSITION_CATALOG.forEach((entry) => {
      expect(within(panel).getByText(entry.rule)).toBeInTheDocument();
    });
    expect(within(panel).getAllByRole('radio')).toHaveLength(6);
  });

  it('模板文案与方法论封闭枚举一致', () => {
    expect(TRANSITION_CATALOG.map((entry) => entry.rule)).toEqual([
      '音频预接',
      '螺口顺滑过渡',
      '卡点硬切',
      'BGM升调截断',
