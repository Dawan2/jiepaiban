...
import { TRANSITION_TEMPLATES, seamOf, type TransitionMethodId } from '../domain/transitions';
import { draftHasSeam, type BeatDraft } from './draft';

interface TransitionPanelProps {
  draft: BeatDraft;
  nextBeatName: string | null;
  onMethodChange: (method: TransitionMethodId | null) => void;
        ...
        <>
          <p className="transition__seam">
            ...
            <span className="transition__seam-flow">
              {draft.name} → {nextBeatName ?? `节拍${draft.index + 1}`}
            </span>
          </p>

          <fieldset className="transition__set">
            <legend className="transition__legend">衔接手法（模板库 6 选 1）</legend>
            <ul className="templates">
              {TRANSITION_TEMPLATES.map((template) => (
                <li key={template.id}>
                  <label
                    className={`template${
                      draft.transitionMethod === template.id ? ' template--on' : ''
                    }`}
                  >
                    <input
                      type="radio"
                      ...
                      value={template.id}
                      checked={draft.transitionMethod === template.id}
                      onChange={() => onMethodChange(template.id)}
                    />
                    <span className="template__label">{template.label}</span>
                    <span className="template__hint">{template.hint}</span>
                  </label>
                </li>
              ))}
            </ul>
          ...
          <textarea
            ...
            value={draft.transitionNote}
            onChange={(event) => onNoteChange(event.target.value)}
          />

          <p className="panel__rule panel__rule--warn">
            组内交给 AI，组间交给人。衔接写进 Prompt 只会让模型在段内乱加黑场，污染素材。
          </p>
        </>
      )}
    </section>
