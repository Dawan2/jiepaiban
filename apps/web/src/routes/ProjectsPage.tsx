    ...
    <li className={`card${summary.archived ? ' card--archived' : ''}`}>
      <Link to={`/p/${summary.id}`} className="card__link">
        ...
        <dl className="card__meta">
          ...
          <div>
            <dt>画幅</dt>
            <dd>{summary.aspect_ratio}</dd>
          </div>
          <div>
            <dt>更新</dt>
            <dd>{formatDate(summary.updated_at)}</dd>
          </div>
        </dl>
        <div className="card__footer">
          <span
            className="dots"
            aria-label={`五节拍完成度 ${summary.filled_beats}/${summary.total_beats}`}
          >
            {BEAT_INDEXES.map((index) => (
              <span
                key={index}
                className={`dots__dot${index <= summary.filled_beats ? ' dots__dot--on' : ''}`}
              />
            ))}
          </span>
          <span className="card__count">
            {summary.filled_beats}/{summary.total_beats} 节拍已填
            {summary.video_count > 0 && ` · ${summary.video_count} 段已生成`}
          </span>
        </div>
      </Link>
  ...
  return (
    <div className="panel panel--danger" role="alertdialog" aria-label="确认删除项目">
      ...
      <p className="panel__desc">
        该项目已有 {summary.video_count} 段生成视频。删除后本地数据无法恢复，建议先导出备份。
      </p>
