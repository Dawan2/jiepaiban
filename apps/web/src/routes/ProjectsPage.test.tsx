...
/** 按板序改板，返回新项目；板经重铸，结构锁跟着回来。 */
function patchBeats(
  source: StoredProject,
  mutate: (beat: ReturnType<typeof rebuildBeat>, at: number) => void,
): StoredProject {
  const { beat_list: stored, ...fields } = source;
  const beats = stored.map((beat, at) => {
...
describe('复用项目（PRD §7.2）', () => {
  it('复制结构与参数、清空画面文案与生成结果', async () => {
    const source = withVideos(
      patchBeats(makeProject('prj_1', { name: '第一集' }), (beat) => {
        beat.plot_core = '上一集的剧情';
        beat.status = 'generated';
        beat.frames.forEach((frame) => {
          frame.text = `上一集第 ${frame.order} 格`;
        });
      }),
      5,
    );
    ...
    const copyId = summaries.find((summary) => summary.reused_from_id === 'prj_1')?.id ?? '';
    const copy = await repository.load(copyId);

    expect(copy?.beat_list).toHaveLength(BEAT_COUNT);
    expect(copy?.beat_list.flatMap((beat) => beat.frames.map((frame) => frame.text))).toEqual(
      Array.from({ length: TOTAL_FRAME_COUNT }, () => ''),
    );
    ...
    // 源项目不受影响。
    expect((await repository.load('prj_1'))?.beat_list[0]?.frames[0]?.text).toBe('上一集第 1 格');
  });
});
...
describe('列表展示', () => {
  it('卡片显示完成度点阵（分母恒为 5）与已生成段数', async () => {
    const repository = await seedRepository([
      withVideos(
        patchBeats(makeProject('prj_1', { name: '第一集' }), (beat, at) => {
          if (at < 3) {
            beat.status = 'filled';
          }
        }),
  ...
  it('参数默认值来自基准表：单集 88 秒 / 五板 8·17·20·25·18', async () => {
    ...
    expect(stored?.total_duration_sec).toBe(88);
    expect(stored?.beat_list.map((beat) => beat.duration_sec)).toEqual([8, 17, 20, 25, 18]);
    expect(BASE_INPUT.aspect_ratio).toBe('9:16');
  });
});
