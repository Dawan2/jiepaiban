 ...
 *
 * 落库由路由壳负责：本视图只把队列状态经 {@link ExportViewProps.onStatesChange} 报上去，
 * 自己不认识仓储。段卡的「已生成」以传进来的项目为底（落库的 `video_url`），
 * 队列只覆盖本次会话真正跑过的那几块板——详见 `segments.ts` 的两源合流。
 */
...
export interface ExportViewProps {
  ...
  readonly now?: () => string;
  /**
   * 队列状态变化时上报，供路由壳把 `video_url` / `prompt_final` 落库。
   * 不传即「只展示不落库」——本视图不认识仓储，落库判据在 `store/generated.ts`。
   */
  readonly onStatesChange?: (states: readonly GenerateBoardState[]) => void;
}
