 ...
 *
 * `prompt_final` 优先取段卡上的**提交快照**（落库的 `prompt_final` 或本次任务的
 * `prompt_snapshot`），只有从未生成过的板才按当前字段现算。快照可溯的意义正在于此：
 * 生成之后又改过文案时，档案里记的是**当时真的发出去的那份**，而不是现在的样子（AC-6.9）。
 */
...
export interface ExportedBeat {
  ...
  /** 提交给模型的 Prompt 全文（所见即所发，R5）；已生成的板给的是提交快照。 */
  readonly prompt_final: string;
  /** `true` 表示 `prompt_final` 是落库 / 任务快照，`false` 表示按当前字段现算。 */
  readonly prompt_is_snapshot: boolean;
...
function exportBeat(project: Project, card: SegmentCard): ExportedBeat {
  ...
  return Object.freeze({
    ...
    status: card.status,
    prompt_final: card.prompt_snapshot ?? assemblePrompt(project, beat),
    prompt_is_snapshot: card.prompt_snapshot !== null,
