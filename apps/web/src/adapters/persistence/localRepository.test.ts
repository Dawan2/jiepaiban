...
import { ProjectLockError, rebuildBeat } from './locks';
...
describe.each(drivers)('LocalRepository（%s 驱动）', (_label, makeDriver) => {
  ...
  it('保存后能按 id 读回完整的 5 板结构', async () => {
    ...
    expect(loaded?.beat_list).toHaveLength(BEAT_COUNT);
    expect(loaded?.beat_list.map((beat) => beat.frame_count)).toEqual([3, 3, 3, 3, 2]);
  });
  ...
  it('列表按更新时间倒序，并投影出完成度与视频数', async () => {
    ...
    expect(second?.total_beats).toBe(BEAT_COUNT);
    expect(second?.video_count).toBe(2);
    expect(second?.filled_beats).toBe(2);
  });
  ...
  it('归档态与复用来源随项目一起落盘', async () => {
    ...
    expect(byId.get('prj_2')?.reused_from_id).toBe('prj_1');
  });
  ...
  it('结构违规的项目拒绝落库（第 6 块板进不来）', async () => {
    const plain = asPlain(project('prj_1'));
    const sixth = plain.beat_list[0];
    if (sixth === undefined) {
      throw new Error('缺少第 1 块板');
    }
    (plain as { beat_list: unknown }).beat_list = [...plain.beat_list, sixth];

    await expect(repository.save(plain)).rejects.toThrow(ProjectLockError);
    expect(await repository.load('prj_1')).toBeNull();
  });

  it('衔接手法泄漏进 Prompt 快照时拒绝落库（RULE-9 / AC-6.4）', async () => {
    const leaked = asPlain(project('prj_1'));
    const first = leaked.beat_list[0];
    if (first === undefined) {
      throw new Error('缺少第 1 块板');
    }
    (first as { prompt_final: string }).prompt_final = `…时长8秒，${first.transition_rule}`;

    await expect(repository.save(leaked)).rejects.toThrow(/衔接手法泄漏进 Prompt 快照/);
  });
  ...
  it('导出封套带版本号，导入后逐字还原', async () => {
    ...
    expect((await restored.load('prj_1'))?.beat_list[0]?.video_url).toBe(
  ...
  it('JSON 文本往返：导出的文本能原样导入', async () => {
    ...
    expect(count).toBe(1);
    expect((await restored.load('prj_1'))?.beat_list).toHaveLength(BEAT_COUNT);
  });
  ...
  it('导入违规备份时整批拒绝，不做部分导入', async () => {
    ...
    const bad = asPlain(project('prj_bad'));
    (bad as { beat_list: unknown }).beat_list = bad.beat_list.slice(0, 4);
    const envelope = {
      kind: ENVELOPE_KIND,
...
describe('落库封套与迁移（架构 §5 持久化与迁移）', () => {
  it('读回时把宫格数归一到锁定值（历史数据修复）', async () => {
    ...
    await driver.clear();
    const stale = asPlain(project('prj_1'));
    // 绕过仓储直接塞一条 B5 记为 3 宫格的旧数据。
    stale.beat_list.forEach((beat) => {
      (beat as { frame_count: number }).frame_count = 3;
    });
    await driver.put('prj_1', stale);
